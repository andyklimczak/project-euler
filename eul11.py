def left(input):
    ...
    for i in range(len):
        for j in range(len - 3):
            if input[i][j] * input[i][j+1] * input[i][j+2] * input[i][j+3] > max:
...
def down(input):
    ...
    for i in range(len-3):
        for j in range(len):
            if input[i][j] * input[i+1][j] * input[i+2][j] * input[i+3][j] > max:
                max = input[i][j] * input[i+1][j] * input[i+2][j] * input[i+3][j]
    return max

def diagonaldown(input):
    max = 0
    for i in range(len-3):
        for j in range(len - 3):
            if input[i][j] * input[i+1][j+1] * input[i+2][j+2] * input[i+3][j+3] > max:
                max = input[i][j] * input[i+1][j+1] * input[i+2][j+2] * input[i+3][j+3]
    return max

def diagonalup(input):
    max = 0
    for i in range(len-1,3, -1):
        for j in range(len-3):
            if input[i][j] * input[i-1][j+1] * input[i-2][j+2] * input[i-3][j+3] > max:
                max = input[i][j] * input[i-1][j+1] * input[i-2][j+2] * input[i-3][j+3]
    return max
...
len = 20
input = [[0 for col in range(20)] for row in range(20)]
...
for i, row in enumerate(input):
    temp = input[i].split()
    input[i] = list(map(int, temp))

print(left(input))
print(down(input))
print(diagonaldown(input))
print(diagonalup(input))